...
# refer to: https://keras.io/api/applications/vgg/#vgg16-function
...
def make_model(arg_input_shape, arg_num_classes):
    ...
    x = inputs
    x = keras.applications.vgg16.preprocess_input(x)  # convert the input images from RGB to BGR, then will zero-center each color channel with respect to the ImageNet dataset, without scaling.
